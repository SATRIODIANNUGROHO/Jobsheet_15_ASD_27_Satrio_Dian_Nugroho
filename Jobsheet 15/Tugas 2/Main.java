package com.jobsheet15.tugas02;
import java.util.*;
public class Main {
        ...
        while (kondisi) {
            switch (menu()) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    hapusDataMahasiswa(mahasiswa27s);
                    break;
                case 6:
                    kondisi = false;
                    break;
    ...
    public static void hapusDataMahasiswa(List<Mahasiswa27> mahasiswa27s) {
        if (mahasiswa27s.isEmpty()) {
            System.out.println("+-------------------+");
        ...
        System.out.println("---------------------------------------------------");
        boolean found = false;
        for (Iterator<Mahasiswa27> iterator = mahasiswa27s.iterator(); iterator.hasNext();) {
            Mahasiswa27 mhs = iterator.next();
            if (mhs.getNim().equals(nimHapus)) {
                iterator.remove();
                found = true;
                System.out.println("\nData Mahasiswa dengan NIM " + nimHapus + " berhasil dihapus.");
                break;
            }
        }
        if (!found) {
            System.out.println("\nData Mahasiswa dengan NIM " + nimHapus + " tidak ditemukan.");
        }
    }
}